...
export default function App() {
  ...
  const activeChannels = useMemo(() => {
    ...
    if (activeTab === 'live-tv') return allBroadcast.sort((a, b) => a.number - b.number);
    if (activeTab === 'my-channel') {
      const connectedCams = MOCK_CHANNELS.filter(c => c.isThirdParty && c.brandId && preferences.connectedCams.includes(c.brandId));
      let filteredBroadcast = allBroadcast;
      if (preferences.favoriteGenres.length > 0) {
        filteredBroadcast = allBroadcast.filter(c => preferences.favoriteGenres.includes(c.currentProgram.genre));
        if (filteredBroadcast.length === 0) filteredBroadcast = allBroadcast;
      }
  ...
  const connectedCamChannels = useMemo(() => MOCK_CHANNELS.filter(c => c.isThirdParty && c.brandId && preferences.connectedCams.includes(c.brandId)), [preferences.connectedCams]);
  ...
  const applyMyChannelRecommendation = useCallback(async (isManualSync: boolean = false) => {
    ...
    const { channelId } = await getRecommendedChannel(activeChannels, preferences);
    setLastSyncTime(new Date());
    if (channelId !== currentChannelId) changeChannel(channelId);
  }, [isMyChannelActive, preferences, currentChannelId, changeChannel, activeChannels]);

  // Effect to dynamically set the recommendation reason based on the current channel
  useEffect(() => {
    if (!isMyChannelActive || !currentChannel || currentChannel.isThirdParty) {
      setRecommendationReason('');
      return;
    }
    if (preferences.favoriteGenres.includes(currentChannel.currentProgram.genre)) {
      setRecommendationReason(`'${currentChannel.currentProgram.genre}'`);
    } else {
      setRecommendationReason('');
    }
  }, [currentChannel, preferences.favoriteGenres, isMyChannelActive]);

  // Initial recommendation and Auto-sync handler
  useEffect(() => {
    if (isMyChannelActive) applyMyChannelRecommendation();
    if (preferences.enableAutoSync && isMyChannelActive) {
      const intervalId = setInterval(() => applyMyChannelRecommendation(), preferences.autoSyncInterval * 60 * 1000);
  ...
  useEffect(() => {
    ...
    if (shuffleableChannels.length < 2) return;
    const advanceInterval = setInterval(() => {
      const currentIndex = shuffleableChannels.findIndex(c => c.id === currentChannelId);
      const nextIndex = currentIndex !== -1 ? (currentIndex + 1) % shuffleableChannels.length : 0;
  ...
  const triggerSimulatedAlert = useCallback(() => {
    ...
    const newAlert: AlertEvent = {
      id: Date.now().toString(), type: 'sound', message: `${randomCam.name}에서 움직임이 감지되었습니다.`,
      channelId: randomCam.id, timestamp: new Date(), severity: 'high'
    };
  ...
  const handleAlertButtonClick = () => {
    ...
    else triggerSimulatedAlert();
  };

  useEffect(() => {
    const intervalId = setInterval(triggerSimulatedAlert, 60000);
    return () => clearInterval(intervalId);
  ...
  const renderMainContent = () => {
    if (activeTab === 'my-channel' || activeTab === 'live-tv') {
      return (
        <>
          <VideoPlayer channel={currentChannel} isChangingChannel={isChangingChannel} />
          <OSD
    ...
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-gray-900 text-white">
        <h1 className="text-4xl font-bold mb-4">{activeTab === 'home' ? '홈' : '기타 메뉴'}</h1>
        <p className="text-gray-400">이 메뉴는 데모 버전에서 제공되지 않습니다.</p>
        <button onClick={() => setActiveTab('my-channel')} className="mt-8 px-6 py-3 bg-blue-600 hover:bg-blue-500 rounded-full font-medium transition-colors">
          마이 채널로 돌아가기
        </button>
      </div>
    );
  ...
  return (
    <div className="flex w-full h-full bg-black font-sans overflow-hidden">
      <NavBar activeTab={activeTab} onTabChange={setActiveTab} onOpenSettings={() => setIsSettingsOpen(true)} />
      <div className="flex-1 relative">
        {renderMainContent()}
        <SmartAlertPiP
          alert={activeAlert}
          camChannel={activeAlert ? MOCK_CHANNELS.find(c => c.id === activeAlert.channelId) : undefined}
          onAccept={(id) => { setActiveTab('my-channel'); changeChannel(id); setActiveAlert(null); }}
          ...
          <div className="absolute top-4 right-4 z-20 flex gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-bold flex items-center gap-1 backdrop-blur-sm transition-colors duration-300 disabled:bg-gray-600 disabled:cursor-not-allowed ${
                activeAlert ? 'bg-red-600/80 text-white hover:bg-red-500' : 'bg-black/50 text-white/60 hover:bg-white/20'
              }`}
